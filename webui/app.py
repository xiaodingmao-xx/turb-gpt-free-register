...
def _registration_password_value(row: dict) -> str:
    """从账号扩展信息中读取注册密码；列表接口只返回是否存在。"""
    direct = str(row.get("registration_password") or "").strip()
    if direct:
        return direct
...
def _attach_password_setup_queue(items: list[dict], queue: dict | None = None) -> list[dict]:
    queue = queue or password_setup_task_service.queue_settings()
    positions = queue.get("positions") if isinstance(queue, dict) else {}
    positions = positions if isinstance(positions, dict) else {}
    for item in items:
        if str(item.get("password_setup_status") or "") == "queued":
            position = positions.get(str(item.get("id")))
            if position is not None:
                item["password_setup_queue_position"] = int(position)
    return items
...
def create_app(auth_code: str | None = None) -> Flask:
    ...
    try:
        recovered = svc.reconcile_stale_jobs()
        if recovered.get("total"):
            logger.warning(
                "[WebUI] 启动时清理历史任务：cancelled=%s stopped=%s",
                recovered.get("cancelled", 0),
                recovered.get("stopped", 0),
            )
    except Exception:
        logger.exception("[WebUI] 启动时清理历史任务失败")
    ...
    def api_accounts():
        ...
        email_filter = str(request.args.get("email", default="") or "").strip()
        source_filter = str(request.args.get("source", default="") or "").strip()
        sort_key = str(request.args.get("sort", default="id") or "id").strip()
        sort_order = str(request.args.get("order", default="desc") or "desc").strip()
        ...
        if paged or page_arg is not None or page_size_arg is not None:
            ...
            queue = password_setup_task_service.queue_settings()
            result["items"] = _attach_password_setup_queue(
                [_compact_account_for_list(r) for r in (result.get("items") or [])],
                queue,
            )
            result["password_setup_queue"] = queue
    ...
    def api_account_password_setup(acc_id: int):
        """给单个账号提交添加/重设密码任务；密码只进入后台任务，不进入响应。"""
        data = request.get_json(silent=True) or {}
        if not db.get_account(acc_id):
            return jsonify({"ok": False, "error": "账号不存在"}), 404
        mode = str(data.get("mode") or password_setup_task_service.default_password_setup_mode()).strip()
    ...
    def api_accounts_password_setup_bulk():
        """批量提交设置密码任务；每个账号独立返回入队/跳过结果。"""
        data = request.get_json(silent=True) or {}
        ids = data.get("account_ids") or data.get("ids") or []
        if not isinstance(ids, list) or not ids:
            return jsonify({"ok": False, "error": "account_ids 必须是非空数组"}), 400
        if len(ids) > 500:
            return jsonify({"ok": False, "error": "单次最多设置 500 个账号"}), 400
        mode = str(data.get("mode") or password_setup_task_service.default_password_setup_mode()).strip()
        password = str(data.get("password") or "")
        started = []
        skipped = []
        ...
        for raw in ids:
            ...
            if queued.get("accepted"):
                started.append(safe)
            elif queued.get("skipped"):
                skipped.append({"id": acc_id, "email": safe.get("email"), "reason": safe.get("error") or "账号密码已经设置"})
            elif queued.get("busy"):
                skipped.append({"id": acc_id, "email": safe.get("email"), "reason": safe.get("error") or "该账号正在设置密码"})
            else:
                failed.append({"id": acc_id, "email": safe.get("email"), "error": safe.get("error") or "入队失败"})
        return jsonify({
            "ok": True,
            "started": started,
    ...
    @app.get("/api/accounts/password-setup-status")
    def api_accounts_password_setup_status():
        """返回账号页轮询所需的设置密码状态，不返回密码。"""
    ...
    def api_account_password_setup_log():
        """读取某邮箱最近一次设置密码日志。?email=xxx"""
        email = (request.args.get("email") or "").strip()
        if not email:
            return jsonify({"ok": False, "error": "email 为空"}), 400
        p = password_setup_task_service.password_setup_log_path(email)
        if not p.exists():
            row = db.get_account_by_email(email) or {}
