...
def reconcile_stale_jobs() -> dict[str, int]:
    ...
    线程池和 stop event 都是进程内状态，WebUI 重启后不会恢复旧任务；
    因此旧的 pending/running/stopping 记录不能继续显示为活动任务。
    当前进程已经登记到 _ACTIVE_JOBS 的任务不会被处理，便于测试或热重载场景复用。
    """
    with _STOP_LOCK:
    ...
    for job in db.list_jobs(limit=10000):
        ...
        if status == "pending":
            ...
            reason = "项目重启后检测到历史排队任务，已自动取消"
            cancelled += 1
        elif status in {"running", "stopping"}:
            new_status = "stopped"
            reason = "项目重启后未找到任务实例，已自动停止历史任务"
            stopped += 1
        else:
            continue

        now = datetime.now().isoformat(timespec="seconds")
        db.update_job(job_id, status=new_status, completed_at=now, error=reason)
        _release_unconsumed_job_email(str(job.get("email") or "").strip() or None, reason)
        _append_job_log(job_id, reason)
        logger.warning("[Service] 清理历史任务 #%s: %s", job_id, reason)

    return {"cancelled": cancelled, "stopped": stopped, "total": cancelled + stopped}
