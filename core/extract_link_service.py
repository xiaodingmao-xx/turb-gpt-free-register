...
from core import db
from core.payment_method_detector import detect_extract_payment_session
        ...
        for event, data in _iter_sse_events(job_id=job_id, cdk=cdk):
            ...
            if event == "log":
                ...
                if not isinstance(result, dict):
                    result = {}
                try:
                    payment_detection = detect_extract_payment_session(
                        result,
                        billing_country=str(
                            result.get("billing_country")
                            or result.get("billingCountry")
                            or ""
                        ),
                        fallback_currency=str(result.get("currency") or ""),
                        expected_method=link_type,
                    )
                except Exception as exc:
                    logger.debug("提链支付会话识别失败: %s", exc, exc_info=True)
                    payment_detection = {
                        "detected": False,
