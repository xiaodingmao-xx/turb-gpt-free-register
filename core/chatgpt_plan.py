...
def resolve_plan_check_route(explicit_proxy: Optional[str] = None) -> dict:
    ...
    selected = normalize_proxy_url(selected)
    if not selected:
        if mode == "proxy":
...
def _first_nested_value(value: Any, keys: tuple[str, ...], *, depth: int = 0) -> str | None:
    """Find the first non-empty scalar value for one of the known region keys."""
    if depth > 5:
        return None
    if isinstance(value, dict):
        ...
        for item in value.values():
            if isinstance(item, (dict, list)):
                found = _first_nested_value(item, keys, depth=depth + 1)
                if found:
                    return found
...
def _extract_plan_eligibility_region(item: dict[str, Any]) -> dict[str, str | None]:
    country_keys = ("country", "country_code", "countrycode", "billing_country", "billingcountry")
    region_keys = (
        "region",
        "region_code",
        "regioncode",
    ...
    country = _first_nested_value(item, country_keys)
    region = _first_nested_value(item, region_keys)
    return {
        "plan_eligibility_country": country.upper() if country else None,
        "plan_eligibility_region": region,
        "plan_eligibility_region_source": "accounts_check" if country or region else None,
    }


def _detect_plan_exit_geo(session) -> dict[str, str]:
    """Best-effort GeoIP lookup using the same HTTP session and final route."""
    try:
    ...
    for endpoint in endpoints:
        try:
            ...
            result = {
                "plan_exit_ip": str(data.get("ip") or data.get("query") or "").strip(),
                "plan_exit_country": country,
                "plan_exit_region": str(data.get("region") or data.get("regionName") or "").strip(),
                "plan_exit_city": str(data.get("city") or "").strip(),
                "plan_exit_timezone": str(data.get("timezone") or "").strip(),
                "plan_exit_geo_source": urlparse(str(endpoint)).netloc or str(endpoint),
            }
            return {key: value for key, value in result.items() if value}
        except Exception as exc:
            logger.debug("套餐查询出口 GeoIP 失败 endpoint=%s: %s", endpoint, exc)
    return {}
...
def parse_accounts_check(data: dict, *, token: str = "") -> dict:
    ...
    eligibility_region = _extract_plan_eligibility_region(item)
    ...
    result.update(eligibility_region)
