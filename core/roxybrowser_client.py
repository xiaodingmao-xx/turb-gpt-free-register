...
# RoxyBrowser 的 /browser/create 不支持并发创建环境。
# 只串行化创建请求，创建完成后各任务仍可独立打开和使用自己的环境。
...
def _saved_proxy_row_to_info(row: dict, check_channel: str = "") -> dict:
    """将 Roxy /proxy/list 的一行转换为选择已保存代理的 proxyInfo。"""
    if not isinstance(row, dict):
        raise ValueError("Roxy 代理列表项格式无效")
    raw_id = row.get("id") or row.get("moduleId") or row.get("module_id")
    try:
        module_id = int(raw_id)
    except (TypeError, ValueError):
        module_id = 0
    if module_id <= 0:
        raise ValueError("Roxy 代理列表项缺少有效代理 ID")

    protocol = str(row.get("protocol") or row.get("proxyCategory") or "SOCKS5").upper()
    if protocol not in {"HTTP", "HTTPS", "SOCKS5", "SSH"}:
        protocol = "SOCKS5"
    ip_type = str(row.get("ipType") or "IPV4").upper()
...
def _proxy_country_matches(row: dict, country: str) -> bool:
...
class RoxyBrowserClient:
    ...
    def list_proxies(self, page_index: int = 1, page_size: int | None = None) -> list[dict]:
        """读取 Roxy 已保存代理资源，返回 /proxy/list 的 rows。"""
        path = str(getattr(_cfg, "ROXY_PROXY_LIST_PATH", "/proxy/list") or "/proxy/list").strip()
        size = page_size or int(getattr(_cfg, "ROXY_PROXY_LIST_PAGE_SIZE", 100) or 100)
    ...
    def _pick_saved_proxy_info(self) -> tuple[dict, dict]:
        ...
        return _saved_proxy_row_to_info(
            row,
            check_channel=str(getattr(_cfg, "ROXY_PROXY_CHECK_CHANNEL", "") or "").strip(),
        ), row
    ...
    def create_profile(self, payload: dict | None = None) -> str:
        ...
        if bool(getattr(_cfg, "ROXY_USE_SAVED_PROXY_POOL", False)) and not body.get("proxyInfo"):
            proxy_info, proxy_row = self._pick_saved_proxy_info()
            body["proxyInfo"] = proxy_info
            logger.info(
                "[Roxy] 使用已保存代理：moduleId=%s country=%s lastIp=%s protocol=%s",
                proxy_info.get("moduleId"),
                proxy_row.get("lastCountry") or "-",
                proxy_row.get("lastIp") or "-",
        ...
        with _ROXY_CREATE_LOCK:
            logger.info("[Roxy] 已获得环境创建锁：%s", body.get("name") or "-")
    ...
    def open_profile(self, profile_id: str | None = None, *, allow_existing_profile: bool = False) -> RoxyOpenResult:
        ...
        if one_profile and configured_pid and not allow_existing_profile:
            raise RuntimeError(
