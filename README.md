...
如果注册流程本身走的是 OTP-only，Roxy 可以在注册成功后继续使用同一个浏览器环境完成密码设置。该流程在 `chatgpt.com` 页面同源执行 CSRF 和重新认证请求，然后等待邮箱 OTP，最后填写新密码，不需要在 `feiyangka.com` 页面跨域调用 ChatGPT 接口。

在 `.env` 或 WebUI「配置 → RoxyBrowser」中设置：

```env
ROXY_PASSWORD_SETUP_ENABLED="True"
ROXY_PASSWORD_SETUP_MODE="post_login_add_password"
ROXY_PASSWORD_SETUP_PASSWORD=""
ROXY_PASSWORD_SETUP_TIMEOUT="120"
```

`post_login_add_password` 用于添加密码，`post_login_password_reset` 用于修改密码。目标密码留空时优先使用 `REGISTER_PASSWORD`，仍为空则为每个账号生成随机密码，并保存到账号记录的 `registration_password` 字段。此功能依赖邮箱 OTP 自动收取配置；密码设置失败会让该注册任务失败并写入对应日志。
...
`ROXY_PASSWORD_SETUP_WORKERS=1` 表示串行；设置为 `2` 或更高时，多个独立 Roxy 环境可以同时修改密码。Roxy 环境创建接口仍使用串行锁，Selenium driver 不会跨线程传递。建议先使用 `2`，确认 Roxy 资源、代理和邮箱 OTP 稳定后再提高。
