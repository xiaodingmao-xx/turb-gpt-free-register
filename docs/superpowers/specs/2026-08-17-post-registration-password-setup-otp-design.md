...
`exclude_codes` 只作为缺少可靠消息身份的邮箱来源的退化手段。GenericAPI 有消息 ID 或
时间戳时，不使用注册验证码值排除设置密码候选。
...
本设计把 `ROXY_PASSWORD_SETUP_MAX_RETRIES` 明确定义为“首次后台执行之外允许追加的重试
次数”。值为 3 时，后台最多执行 4 次（首次执行 + 3 次重试）。现有代码把该配置近似当成
总尝试次数，实施时必须同步调整计数、日志和测试，避免同名配置继续存在两种语义。
...
- 后台可恢复错误按 15/60/180 秒重新排到队尾且不占 worker。
